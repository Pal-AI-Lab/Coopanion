...
2. **语音输入**:默认开着,用的是 Windows 自带的语音识别,不用下载。
   Windows 第一次会问能不能用麦克风,选允许。之后**按住右 Ctrl 说话**,松开就算一句;Coo 会歪头听,
   并在虚线气泡里边听边显示听到的字(灰色的是还没听完、可能会改的部分)。嫌识别不够准,可以在左栏「语音输入」页把识别引擎换成 whisper.cpp,
   点「下载并启动」取回识别程序和中文模型(约 190 MB,只下一次)。
...
- **装扮**:换配色、帽子、耳饰、眼镜、颈饰;
- **语音输入**:开关、识别引擎(Windows 自带或 whisper.cpp)、换说话键、换麦克风、收音方式、电平条和听到的内容。
...
- **说话**:按住右 Ctrl 说话;或者把鼠标停在桌宠身上点气泡按钮、双击桌宠,都能打字。
  设置窗口的「语音输入」页里可以换说话键、换麦克风,或者改成按一下开关、一直收音,电平条显示实时音量。
- **麦克风按钮**:鼠标停在桌宠身上,身旁下面那个按钮是语音输入的开关,点一下开或关;
  正在听你说话时**长按**它,不等你停顿,听到的这句话马上发给 Coo。
- **黑白模式**:右键菜单里的「黑白模式」切换;默认是黑色(夜间)模式。
...
- **提问**:Coo 有时会冒出带选项的问题,点选项或直接按键盘上的 1–3(选项出来时桌宠接过键盘,答完还给你原来的窗口);不想选就在最后一格自己写。
- **装扮**:设置窗口的「装扮」页(右键菜单里的「装扮」也打开这一页),换配色、帽子、耳饰、眼镜、颈饰,改动立刻生效。
...
| 运行日志 | `data\logs` |
| 语音识别程序与模型(换成 whisper.cpp 时才下载) | `data\home\runtimes`、`models` |
...
- **听不到我说话**:确认桌宠身旁的麦克风按钮没被划掉、「语音输入」页的识别服务显示就绪、说话时按住了说话键(默认右 Ctrl),以及 Windows 设置 → 隐私和安全性 →
  麦克风里允许了桌面应用。「语音输入」页的电平条随声音跳动,说明麦克风选对了。
  识别服务报「没有语音识别器」时,到 Windows 设置 → 时间和语言 → 语言,给中文装上「语音识别」,或者换成 whisper.cpp。
...
MIT。随附或运行时下载的第三方组件:Electron(MIT)、Cortico(MIT)、whisper.cpp 与其 ggml 模型(MIT,选用时下载)、koffi(MIT)、jpeg-js(BSD-3-Clause)、pnpm(MIT)。
