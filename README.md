...
- **陪你聊天**:按住说话键说话(Windows 是右 Ctrl,Mac 是右 Option),或者直接打字,Coo 在气泡里回你。语音用 FunASR 在你电脑上识别。
...
> 麦克风(语音输入)、输入监控(按住说话键)、录屏与系统录音和辅助功能(让 Coo 操作电脑)。
> 不想让它碰电脑,后两项不给就行。改了「输入监控」「辅助功能」「录屏」之后要重启 CortiCompanion 才生效。

## 快速上手

1. **启动**:屏幕底边会出现 Coo,任务栏右下角(Mac 是屏幕顶部菜单栏)多一个图标。设置窗口不会自己弹出来。
2. **连接模型**:Coo 会冒气泡问你要不要去填 API Key,点「去填」。
   选「等会儿」也没关系,只要还没填,它过一阵会再问;你对它说话时也会提醒。
3. **跟着引导走**:设置窗口第一次打开时,Coo 会跳出来和你对话,一问一答把事情办完:
   1. 问你怎么称呼;
   2. 问你希望它安静还是活泼:点「不乱动 / 多待着 / 常走动」,它马上站着不动、溜达起来或者跑来跑去给你看;
   3. 请你粘贴 DeepSeek 的 API Key,当场连一下,连上了它会高兴地跳起来;
   4. 一键下载语音识别模型(FunASR,约 230 MB,从国内的 ModelScope 下载),再告诉你怎么和它说话、按钮在哪;
   5. 告诉你关掉窗口后去哪找它。

   右上角的「跳过」随时可以跳过。之后在「开始」页右上角点「使用引导」能再看一遍。
4. **打个招呼**:按住说话键(Windows **右 Ctrl**,Mac **右 Option**)说「你好」,松开发送。第一次系统会问能不能用麦克风,选「允许」。
...
| 方式 | 怎么做 |
|---|---|
| 语音 | **按住说话键**(Windows 右 Ctrl,Mac 右 Option)说话,松开就算一句。Coo 会歪头听,在虚线气泡里边听边显示听到的字(灰色部分还可能改)。 |
...
模型约 230 MB,第一次用时在引导或「语音输入」页点一下「下载」就行,从 ModelScope 下载(国内直连),下完就一直能用。
