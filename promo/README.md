...
| `electron promo/record.cjs` | 离屏逐帧渲染,经 ffmpeg 输出 `promo/dist/CortiCompanion-promo.mp4`。`--from`/`--to` 只录一段,`--out` 改输出路径,`--fps` 改帧率(默认 30),`--size 2560x1440` 按该分辨率渲染(画面布局不变,约 0.1 秒一帧);ffmpeg 取自 `$FFMPEG`,没有就用 PATH 里的 |
